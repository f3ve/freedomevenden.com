...
const _sfc_main$2 = /* @__PURE__ */ vue.defineComponent({
  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      _push(`<li${serverRenderer.ssrRenderAttrs(vue.mergeProps({ class: "rounded-lg hoverable" }, _attrs))}><a class="block pa-4 decoration-none"${serverRenderer.ssrRenderAttr("href", _ctx.project.link)} target="_blank" rel="noopener"><p class="text-warning ma-none mb-0 text-2xl font-bold font-header">${serverRenderer.ssrInterpolate(_ctx.project.name)}</p><p class="my-0 text-sm font-header font-header text-secondary">${serverRenderer.ssrInterpolate(_ctx.project.role)}</p><p class="my-0 text-op-75 font-header">${serverRenderer.ssrInterpolate(_ctx.project.description)}</p></a></li>`);
    };
  }
});
  ...
  setup(__props) {
    return (_ctx, _push, _parent, _attrs) => {
      ...
      serverRenderer.ssrRenderList(_ctx.projects, (value, key) => {
        _push(`<li class="mb-8"><h2 class="mt-0 w-fit">${serverRenderer.ssrInterpolate(key)}</h2><ul class="blog-list flex flex-col list-none gap-sm border-l-4 fe-border-l"><!--[-->`);
...
_sfc_main.setup = (props, ctx) => {
  const ssrContext = vue.useSSRContext();
  (ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("src/pages/projects/index.md");
